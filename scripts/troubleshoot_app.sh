...
check_pod_health() {
	...
	done < <(kubectl get pods -n "$namespace" -l app="$app_name" -o go-template='{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}' 2>/dev/null || true)
...
get_pod_logs() {
	...
	while IFS= read -r pod_name; do
		...
		kubectl logs "$pod_name" -n "$namespace" 2>/dev/null || echo "Failed to get logs for $pod_name"
		echo ""
	done < <(kubectl get pods -n "$namespace" -l app="$app_name" -o go-template='{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}' 2>/dev/null || true)
}
