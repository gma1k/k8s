...
if [ -z "$task" ]; then
	...
	select namespace in "All namespaces" $(kubectl get ns | awk 'NR>1 {print $1}'); do
		case $namespace in
		"")
	...
	3)
		if [ -z "$namespace" ]; then
			...
			PS3="Please enter your choice: "
			select label in $(kubectl get pods -n "$namespace" --show-labels 2>/dev/null | awk 'NR>1 {print $NF}' | awk -F, '{for (i=1;i<=NF;i++) print $i}' | sort | uniq || echo ""); do
				case "$label" in
				"")
					echo "Invalid choice. Please try again."
					;;
			...
			echo "Listing container images filtering by pod label $label_selector in namespace $namespace."
			kubectl get pods -n "$namespace" -o jsonpath="{.items[*].spec.containers[*].image}" -l "$label_selector" 2>/dev/null || echo "No pods found with label $label_selector"
		fi
		break
		;;
	4)
